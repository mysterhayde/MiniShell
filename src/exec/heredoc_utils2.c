...
/*   Updated: 2025/04/05 00:14:55 by hdougoud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	read_heredoc_lines(t_mini *mini, char *limiter, int temp_fd, int heredoc_num)
{
	...
	if (!prompt)
	{
		prompt = ft_strdup("heredoc> "); //protect malloc
		if (!prompt)
			exit (1);
	}
	while (1)
	{
		...
		line = readline(prompt); //use read in a while for check signal (readline doesnt be stopped by signal)
		if (!line)
		{
