...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/21 13:54:45 by cbopp             #+#    #+#             */
/*   Updated: 2025/03/21 14:14:04 by cbopp            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	exec_redir_heredoc(t_mini *mini, t_token *token)
{
	...
	if (apply_redir_heredoc(mini, token))
...
int	minipipe_heredoc(t_mini *mini)
{
	...
	if (run_pipe_commands_heredoc(mini, &p) == 1)
		return (1);
...
int	exec_paren_with_redir_heredoc(t_mini *mini, t_token *token)
{
	...
	if (has_logical_ops(mini->token))
		...
		ret = exec_redir_heredoc(mini, mini->token);
...
int	exec_logical_with_redir_heredoc(t_mini *mini, t_token *token)
{
	...
	if (!next_op)
		return (exec_redir_heredoc(mini, token));
	if (is_parenthesis_cmd(token))
		ret = exec_paren_with_redir_heredoc(mini, token);
	else
		ret = exec_redir_heredoc(mini, token);
	if (next_op->type == AND_OP)
		return (process_remaining_cmds(mini, next_op->next, ret == 0));
